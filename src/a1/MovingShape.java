...
public abstract class MovingShape {
    ...
    public void setPath(int pathID) {
        switch (pathID) {
            ...
            case 2 : {
                path = new NewPath();
               // System.out.println("New Path selected");
                break;
            }
        }
    }
    ...
    public class BouncingPath extends MovingPath {
        ...
        /** constructor to initialize values for a bouncing path
        */
        public BouncingPath() {
           // deltaY = 1+Math.random(5); <-- this is an error since no parameter in Math.random method
           // deltaX = 1+Math.random(5); <-- this is an error since no parameter in Math.random method
           // So the below code will be used instead
    ...
    public class NewPath extends MovingPath {
        ...
        public NewPath() {
          //get first angle 
          angle = 1.5 * Math.PI - Math.atan2(topLeft.y,topLeft.x); 
          //center point
       ...
       */
       public void move() {
           ...
           //System.out.println("New Path moving");
           if( angle > 360 ) angle = 1.0;
           angle = angle + 0.05;
           
           //System.out.println("New Path moving: "+ angle);
           
           int increaseX = centerX + (int) (radius * Math.cos(angle));
           int increaseY = centerY + (int) (radius * Math.sin(angle));
